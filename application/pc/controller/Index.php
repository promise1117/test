...
class Index extends Base
{
    public function __construct()
    {
        ...
        foreach($this->catList as $k=>$v){
            $cat_two = Db::name('category')->where('parent_id',$v['cat_id'])->where('is_show','1')->select();
    ...
    public function index(){
        ...
        $hotGoods = Db('Goods')->where($hotwhere)->order($hotsort)->select();
...
//        returnResponse(100,'暫無商品,敬請期待！！！');
    ...
    public function pcGetGoodsInfo(){
        ...
        //商品浏览量+1
        $this->goods->where('id',$gid)->setInc('visit');
        //商品内容
...
//        echo $this->goodsinfo->getLastSql();
            ...
            for($i=0;$i<count($join);$i++){
                ...
                foreach ($goodsinfo_bind as $k_=>$v_){
                    if(is_null($goodstc[$k]['goodstc_bind'][$k_]['sell_price'])){
                        $goodstc[$k]['goodstc_bind'][$k_]['sell_price'] = 0;
                    }
    ...
    public function pcGetCategoryList($cat_id,$cat_name){
...
//        ]);
        ...
        $res = Db::name('goods')->where('categoryid',$cat_id)->select();

        $this->assign(
            ['goods'=>$res,'cat_name'=>$cat_name,'catlist'   => $catList]
        );
        return $this->fetch();
    }
